...
class GPUDataset(BaseImageDataset):
    def __init__(self,
                 sampler
        ):
...
class ActionDataset(TrainAndVal):
    """
    ...
    Dataset to provide (s, a, r, s') samples to a torch dataloader. Formerly named dexnex_2cams_image_ql_dataset.py:GPUDataset but that name isn't descriptive.
    ...
    # def compute_weights(self, dataset: GPUDataset):
    #     """
    ...
    def compute_weights2(self, dataset: GPUDataset):
        """
    ...
    def init(self):
        ...
        # make the datasets
        self.train_dataset = GPUDataset(self.train_sampler)
        self.val_dataset = GPUDataset(self.val_sampler)
        self.all_dataset = GPUDataset(self.sampler)
